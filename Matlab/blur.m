I = im2double(imread('./frame0015.jpg'));           %读入清晰原图像
figure(1);
imshow(I,[]);         %显示原图像
title('原图像');

PSF = fspecial('motion', 36, 0);   %建立二维仿真线性运动滤波器PSF
MF = imfilter(I,PSF,'conv', 'circular');  %用PSF产生退化图像
imwrite(MF, "C:/Users/wyya123/Desktop/CodeBase/Python/blur.png")
...
title('维纳滤波复原');
imwrite(wnr, "C:/Users/wyya123/Desktop/CodeBase/Python/wnr.png")

%NP = 0.001 * numel(I);
reg = deconvreg(MF,PSF);
figure(4);imshow(reg,[]);
title('有约束的最小二乘滤波复原');
imwrite(reg, "C:/Users/wyya123/Desktop/CodeBase/Python/reg.png")

invert = deconvblind(MFN, PSF);
figure(5);
imshow(invert,[]);
title('逆滤波复原');
imwrite(invert, "C:/Users/wyya123/Desktop/CodeBase/Python/invert.png")
